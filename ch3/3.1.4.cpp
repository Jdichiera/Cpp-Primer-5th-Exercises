// Rewriting exercise 2.41.2 with using statements
#include <iostream>
...
int main() {
	...
	cout << "Please enter all sales for the same ISBN in the "
			<< "following format: ISBN UNITS_SOLD PRICE" << endl;
	cout << "A sale entered for a different ISBN " 
		<< "will end the program and print out the total " 
		<<  "sales entered for the initial ISBN." << endl << endl;

	cout << "Enter data for the sale: ";
	cin >> currentSale.bookNo >> currentSale.units_sold >> price;
	...
	*/
	...
	while (currentSale.bookNo == totalSale.bookNo) {
		...
		cout << "Enter data for the next sale: ";
		cin >> currentSale.bookNo >> currentSale.units_sold >> price;
		currentSale.revenue = currentSale.units_sold * price;
	}
	...
	*/
	...
	cout << "Total sales for ISBN: " << totalSale.bookNo << endl;
	cout << "(UNITS_SOLD TOTAL_REVENUE AVERAGE_PRICE)" << endl;
	cout << totalSale.units_sold << ' ' 
		<< totalSale.revenue << ' ' << averagePrice << endl;
